...
test('create → pay → fulfilled → duplicate ignored', async ({ page }) => {
  ...
  // 4. The worker fulfills asynchronously; the page refreshes itself. Wait for
  //    the order to reach FULFILLED and an invoice to be issued.
  await expect(page.getByText('FULFILLED', { exact: true })).toBeVisible({ timeout: 20_000 });
  await expect(page.getByText(/INV-\d{8}-/)).toBeVisible();

  // 5. Capture the invoice number, then send a duplicate webhook.
  const invoiceText = await page.getByText(/INV-\d{8}-/).first().textContent();
  ...
  expect(await page.getByText(/INV-\d{8}-/).first().textContent()).toBe(invoiceText);
});
